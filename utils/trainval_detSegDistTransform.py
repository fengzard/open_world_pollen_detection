    ...
    for epoch in range(start_epoch, num_epochs):        
        ...
        for phase in ['train', 'test']:
            ...
            for sample in dataloaders[phase]:
                ...
                # image, grndDistTransform = sample
                
                # grndSeg=grndDistTransform

                image = image.to(device)
                grndSeg = grndSeg.to(device) 
                grndDistTransform = grndDistTransform.to(device)
                ...
                with torch.set_grad_enabled(phase=='train'):
                    ...
                    for ii in range(len(scaleList)):
                        ...
                        loss_reg = lossFunc_reg(grndDistTransform, predDistTransform, grndSeg, mask_overlap)      
            ...
            path_to_save_checkpoint = os.path.join(work_dir, 'epoch-{}.pt'.format(epoch+1))
            checkpoint = { 
                'epoch': epoch,
